...
// ============================================================
// Phase 6: Profit Report - Server Actions
// ============================================================
// 작성일: 2025-01-26
// 목적: 이익 레포트 조회 Server Action
...
/**
 * 이익 레포트 조회 Server Action
 * @param filter - 레포트 필터 조건 (시작일, 종료일, 그룹핑 방식, 지점ID)
 * @returns ProfitReportResponse - 성공 여부, 데이터, 오류 메시지
 */
export async function getProfitReport(
  ...
  try {
    ...
    if (reportError) {
      console.error('Profit report RPC error:', reportError)
      return {
        success: false,
        data: [],
    ...
    const mappedData: ProfitReportRow[] = (reportData || []).map((item: any) => ({
