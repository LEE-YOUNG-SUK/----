...
export default function EditPurchaseModal({ purchase, onClose, onSave }: EditPurchaseModalProps) {
  ...
  useEffect(() => {
    if (taxIncluded) {
      ...
      // 부가세 미포함: 수량 × 단가 = 공급가
      const supply = quantity * unitCost
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div>
              <p className="text-xs text-gray-900 mb-1">공급업체</p>
              <p className="text-sm font-medium text-gray-900">{purchase.client_name}</p>
            </div>
            <div>
              <p className="text-xs text-gray-900 mb-1">입고일</p>
              ...
              <div>
                ...
                <div className="flex items-center">
                  <input
                    ...
                    step="0.01"
                  />
                  <span className="ml-2 text-sm text-gray-900">{purchase.unit}</span>
                </div>
              </div>
