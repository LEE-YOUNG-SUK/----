...
const columnHelper = createColumnHelper<PurchaseGroup>()
const itemColumnHelper = createColumnHelper<PurchaseHistory>()
...
export default function PurchaseHistoryTable({
  ...
  const filteredGroups = useMemo(() => {
    return groupedData.filter((group) => {
      ...
      if (appliedStartDate || appliedEndDate) {
        const groupDate = new Date(group.purchase_date).toLocaleDateString('sv-SE')
      ...
      if (appliedCustomer) {
        ...
        const matchesClient = (group.client_name || '').toLowerCase().includes(cs)
  ...
  const filteredItems = useMemo(() => {
    return data.filter((item) => {
      ...
      if (appliedStartDate || appliedEndDate) {
        const itemDate = new Date(item.purchase_date).toLocaleDateString('sv-SE')
      ...
      if (appliedCustomer) {
        const cs = appliedCustomer.toLowerCase()
        if (
          !(item.client_name || '').toLowerCase().includes(cs) &&
          !(item.notes || '').toLowerCase().includes(cs)
        ) return false
      }
  ...
  const totalAmount = viewMode === 'transaction'
    ? filteredGroups.reduce((sum, g) => sum + g.total_amount, 0)
  ...
  // 품목 기준 컬럼
  const itemColumns = [
    itemColumnHelper.accessor('purchase_date', {
      header: '일자',
      size: 120,
      cell: (info) => (
    ...
    itemColumnHelper.accessor('client_name', {
      header: '거래처',
      size: 140,
      cell: (info) => (
        <span className="text-sm text-gray-900 truncate block">{info.getValue() || '-'}</span>
    ...
    <div className="flex flex-col h-full">
      <ContentCard className="flex flex-col h-full !p-0">
          ...
          {viewMode === 'transaction' ? (
            table.getRowModel().rows.length === 0 ? (
              <div className="px-4 py-12 text-center text-gray-900">
                {(productSearch || customerSearch) ? '검색 결과가 없습니다.' : '입고 내역이 없습니다.'}
              </div>
            ) : (
                ...
                return (
                  <div key={row.id} className="p-3 border-b hover:bg-gray-50 transition">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                      <div>
                        <p className="text-gray-900">공급업체</p>
                ...
                return (
                  <div key={row.id} className="p-3 border-b hover:bg-gray-50 transition">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-xs text-gray-900">{new Date(item.purchase_date).toLocaleDateString('ko-KR')}</span>
                    </div>
        ...
        {viewMode === 'product' && (
          <div className="hidden md:block flex-1 overflow-y-auto">
            <div className="overflow-x-auto">
              <table style={{ width: itemTable.getCenterTotalSize() }} className="min-w-[1300px]">
                <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
