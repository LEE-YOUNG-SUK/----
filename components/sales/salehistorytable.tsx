...
export default function SaleHistoryTable({
  ...
  const columns = [
    ...
    ...(can('sales_management', 'delete') ? [columnHelper.display({
      id: 'delete',
      header: '🗑️',
      size: 60,
      enableSorting: false,
  ...
  const handleDeleteGroup = async (group: SaleGroup) => {
    ...
    const ok = await confirm({
      title: '거래 삭제',
      message: `이 거래건을 삭제하시겠습니까?\n\n거래번호: ${refDisplay}\n품목: ${group.total_items}개\n총액: ₩${group.total_price.toLocaleString()}\n\n해당 거래의 모든 품목이 삭제됩니다.`,
      variant: 'danger'
    })
    if (!ok) return

    setIsDeleting(group.reference_number)
    const toastId = toast.loading(`${label} 삭제 중... (${group.total_items}개 품목)`)

    for (const item of group.items) {
      const result = await deleteSale({ sale_id: item.id })
      if (!result.success) {
    ...
    toast.success(`${label} ${group.total_items}개 품목 삭제 완료 (₩${group.total_price.toLocaleString()})`, { id: toastId })
    setIsDeleting(null)
    router.refresh()
  }
